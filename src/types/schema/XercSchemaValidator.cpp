...
#if _XERCES_VERSION >= 30000
// Too many changes split it in two
// Adapted from Xerces 3.0.1

// ---------------------------------------------------------------------------
//  XercSchemaValidator: Constructors and Destructor
// ---------------------------------------------------------------------------
XercSchemaValidator::XercSchemaValidator( XMLErrorReporter* const errReporter
...
void XercSchemaValidator::faultInAttr (XMLAttr&    toFill, const XMLAttDef&  attDef)   const
{
    //
...
void XercSchemaValidator::reset()
{
    fTrailingSeenNonWhiteSpace.flags = 0;
    fSeenId = false;
...
bool XercSchemaValidator::requiresNamespaces() const
{
    return true;
}

void XercSchemaValidator::validateAttrValue (const XMLAttDef*      attDef
...
void XercSchemaValidator::validateElement(const   XMLElementDecl*  elemDef)
{
...
// modeled after validateElement
void XercSchemaValidator::validateProcessorStipulatedTypeName(const   XMLElementDecl*  elemDef)
{
    fErrorOccurred = false;
...
void XercSchemaValidator::preContentValidation(bool,
...
void XercSchemaValidator::postParseValidation()
{
    //
...
//
void XercSchemaValidator::normalizeWhiteSpace(DatatypeValidator* dV, const XMLCh* const value, XMLBuffer& toFill)
{
...
void XercSchemaValidator::checkParticleDerivationOk(SchemaGrammar* const aGrammar,
...
void
XercSchemaValidator::checkNameAndTypeOK(SchemaGrammar* const currentGrammar,
...
void
XercSchemaValidator::checkRecurseAsIfGroup(SchemaGrammar* const currentGrammar,
...
void
XercSchemaValidator::checkRecurse(SchemaGrammar* const currentGrammar,
                              const ContentSpecNode* const derivedSpecNode,
                              const int derivedScope,
...
bool XercSchemaValidator::checkNSSubsetChoiceRoot(const ContentSpecNode* const derivedSpecNode,
                                    const ContentSpecNode* const baseSpecNode) {
    bool found = false;
...
bool XercSchemaValidator::checkNSSubsetChoice(const ContentSpecNode* const derivedSpecNode,
                                    const ContentSpecNode* const baseSpecNode) {
...
bool
XercSchemaValidator::isWildCardEltSubset(const ContentSpecNode* const derivedSpecNode,
                                     const ContentSpecNode* const baseSpecNode) {
...
void
XercSchemaValidator::checkNSRecurseCheckCardinality(SchemaGrammar* const currentGrammar,
                                                const ContentSpecNode* const derivedSpecNode,
                                                ValueVectorOf<ContentSpecNode*>* const derivedNodes,
                                                const int derivedScope,
...
void
XercSchemaValidator::checkRecurseUnordered(SchemaGrammar* const currentGrammar,
                                       const ContentSpecNode* const derivedSpecNode,
                                       ValueVectorOf<ContentSpecNode*>* const derivedNodes,
                                       const int derivedScope,
                                       ContentSpecNode* const baseSpecNode,
...
void
XercSchemaValidator::checkMapAndSum(SchemaGrammar* const currentGrammar,
                                const ContentSpecNode* const derivedSpecNode,
                                ValueVectorOf<ContentSpecNode*>* const derivedNodes,
                                const int derivedScope,
                                ContentSpecNode* const baseSpecNode,
...
//  XercSchemaValidator: Implementation of the XMLValidator interface
// ---------------------------------------------------------------------------
int XercSchemaValidator::checkContent (XMLElementDecl* const elemDecl
                                 , XERCES_CPP_NAMESPACE::QName** const         children
...
// ---------------------------------------------------------------------------
//  XercSchemaValidator: Validator method
// ---------------------------------------------------------------------------
// Do Schema Normalization depends on the WhiteSpace Facet
// preserve : No normalization is done
// replace  : All occurrences of #x9 (tab), #xA (linefeed) and #xD (carriage return)
...
#endif  // if _XERCES_VERSION >= 30000
